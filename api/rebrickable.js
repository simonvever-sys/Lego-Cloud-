...
export function createRebrickableClient(apiKey = getDefaultApiKey()) {
  ...
  function normalizeSetPart(item, setNumber) {
    return {
      cacheKey: `${setNumber}:${item.part.part_num}:${item.color.name}`,
      setNumber,
  ...
  async function getSetPartsPage(setNumber, page = 1, pageSize = PARTS_PAGE_SIZE) {
    ...
    const cacheKey = getCacheKey("set-parts", `${normalizedSet}:page:${page}:size:${pageSize}`);
    const cached = readCache(cacheKey);
    ...
    const data = await request(
      `/sets/${encodeURIComponent(normalizedSet)}/parts/?page=${page}&page_size=${pageSize}`
    );
